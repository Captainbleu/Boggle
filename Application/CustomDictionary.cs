...
public static class CustomDictionary
{
    ...
    /// </list>
    /// </summary>
    private static SortedDictionary<int, string[]> wordsByLength = new();
    ...
    /// </list>
    /// </summary>
    private static SortedDictionary<char, string[]> wordsByLetter = new();
    ...
    /// </list>
    /// </summary>
    private static SortedDictionary<int, int> wordCountByLength = new();
    ...
    /// </list>
    /// </summary>
    private static SortedDictionary<char, int> wordCountByLetter = new();
    ...
    {
        wordCountByLength.Clear();
        wordCountByLetter.Clear();
        wordsByLength.Clear();
        wordsByLetter.Clear();
    }
                                          ...
                                          SortedDictionary<char, List<string>> tempWordsByLetter)
    {
        ...
        string file = "Files/PossibleWords_" + Language.LanguageCode + ".txt";
                                            ...
                                            SortedDictionary<char, List<string>> tempWordsByLetter)
    {
        ...
        if (!wordCountByLength.TryAdd(length, 1))
        {
            wordCountByLength[length]++;
        }

        if (!wordCountByLetter.TryAdd(initial, 1))
        {
            wordCountByLetter[initial]++;
        }
    }
                                             ...
                                             SortedDictionary<char, List<string>> tempWordsByLetter)
    {
        ...
        {
            wordsByLength.Add(kvp.Key, kvp.Value.ToArray());
        }

        foreach (var kvp in tempWordsByLetter)
        {
            wordsByLetter.Add(kvp.Key, kvp.Value.ToArray());
        }
    }
    ...
    private static void SortLists()
    {
        ...
        {
            QuickSort(kvp.Value, 0, kvp.Value.Length - 1);
        }
        foreach (var kvp in wordsByLetter)
        {
            QuickSort(kvp.Value, 0, kvp.Value.Length - 1);
        }
    }
    ...
    {
        if (wordCountByLength.ContainsKey(word.Length) && wordCountByLetter.ContainsKey(word[0]))
        {
            int countByLength = wordCountByLength[word.Length];
            int countByLetter = wordCountByLetter[word[0]];

            if (countByLength <= countByLetter)
            {
                return BinarySearch(wordsByLength[word.Length], word, 0, countByLength - 1);
            }
            else
            {
                return BinarySearch(wordsByLetter[word[0]], word, 0, countByLetter - 1);
            }
        }
